# Fast Fitting Strategies

This vignette explores practical ways to make `vrcmort` fits faster,
focusing on the no-covariate setting (`~ 1`) and small examples that run
quickly.

## Simulate a tiny example

We use a small dataset that is still large enough for speed differences
between model options to be visible:
...
- **A**: region-specific time random-walk deviations (`time = "region"`
  vs `"national"`)
- **B**: region-varying conflict slopes (`conflict = "region"` vs
  `"fixed"`)
