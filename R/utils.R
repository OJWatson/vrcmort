...
as_int_index <- function(x, name = deparse(substitute(x))) {
  ...
  if (is.numeric(x)) {
    if (any(is.na(x))) {
      stop("`", name, "` contains NA", call. = FALSE)
    }
...
standardise_vector <- function(x, centre = TRUE, scale = TRUE) {
  x <- as.numeric(x)
  if (!centre && !scale) {
    return(list(x = x, centre = 0, scale = 1))
  }
...
as_stan_array <- function(x) {
  ...
  as.array(as.numeric(x))
}

#' @keywords internal
as_stan_array_int <- function(x) {
  if (is.null(x)) {
    return(integer(0))
  }
  as.array(as.integer(x))
}
